...
Every vessel honestly positioned on the schematic repeats its register number beside a compact detail plate containing its name, direction, knots, and ETA. Place all plates in one global collision pass: like matching-polarity magnets, plates may not cover any route ribbon, Brickell or another bridge, another vessel, or another plate. A leader connects each displaced plate to its corresponding hull; if no legal plate position exists, the numbered hull and complete register row remain rather than forcing an overlap. Put two unmistakable chevrons ahead of each hull along its actual route tangent—never behind it and never as a line through the vessel. Selecting a register row emphasizes its matching hull and plate. Hull symbols receive enough space along the schematic route to remain individually legible. The reported position stays a separate dot, with a dashed route connector wherever the symbol is displaced; stored coordinates, distance, time and forecasts remain unchanged. Probable towing units can contain several independently supported tugs and one companion, with a shared route link and a written relationship. Inference uses retained motion rather than vessel names; stale members expire normally.
